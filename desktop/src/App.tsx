    ...
    <div className="app-shell">
      <aside className="sidebar">
        <button className="brand" type="button" onClick={() => setView("overview")} aria-label={t("ENHE Codex Backup")}>
          ...
          <span className="brand-copy"><small className="brand-version">v0.1.8</small><strong>ENHE</strong><small>Codex Backup</small></span>
        </button>
...
function SettingsPage({ headingRef, config, scheduler, onSave, onConfigChange, onNotice, onError }: SettingsPageProps) {
  ...
  return (
    <div className="page">
      ...
      <section className="settings-footer"><button className="primary-button" type="button" onClick={() => { onConfigChange(draft); void onSave(draft); }}>{t("保存设置")}</button><span>{t("当前版本")} 0.1.8</span></section>
    </div>
  );
}
