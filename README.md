...
ENHE Codex Backup 是一个独立的 Windows x64 本地优先Codex项目和数据备份、恢复和离线迁移工具。它可以在没有云端配置、没有 GPT 登录的条件下保存 Codex 资料、项目文件、Git 状态、对话和开发交接资料，并在本机或另一台设备上恢复。0.1.8 增加了项目根目录扫描引导、全盘扫描范围提示、本次结果与保留目录分组以及一键选择本次扫描项目。
