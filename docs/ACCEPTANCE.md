...
| A11 bilingual UI | TypeScript, build, DOM flow tests, operation-guide flowchart, permission summary/admin action, and version label pass; native folder-picker click-through and desktop screenshot/WebView visual check are NOT_RUN | PARTIAL |
| A12 installer/device | 0.1.2 Windows x64 NSIS installer was generated with a matching SHA-256 file, the prior current-user installation was removed, and 0.1.2 launched with a responsive window and versioned title; clean-profile and second device/account tests are NOT_RUN | PARTIAL |
