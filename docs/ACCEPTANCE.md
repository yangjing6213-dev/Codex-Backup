...
| --- | --- | --- |
| A01 local offline | Local path is the default; cloud-off state and no-remote adapter tests pass; Rust/Tauri release compilation, isolated restic acceptance, and the bundled-restic asynchronous synthetic backup/restore round trip pass; installed-app flow is startup-verified | PARTIAL |
...
| A11 bilingual UI | TypeScript, build, DOM flow tests, operation-guide flowchart, permission summary/admin action, version label, and actionable bilingual backup/restore error guidance pass; the installed 0.1.6 startup UI was captured and visually checked; native folder-picker and full click-through remain NOT_RUN | PARTIAL |
| A12 installer/device | 0.1.6 Windows x64 NSIS installer was generated with a matching SHA-256 file, the prior current-user installation was removed, and 0.1.6 launched with matching registry, binary, sidebar, and title-bar versions; clean-profile and second device/account tests are NOT_RUN | PARTIAL |
